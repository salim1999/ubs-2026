...
by an audit or a measured experiment:
  ...
  - Pool across mccs, label afterwards. The same subscription rotates its
    ...
    (timmyo exp 4).
  - Category = majority vote over the members' keyword families
    (category_map.classify), which imputes a family for the keyword-less
    cycles of a rotating description. category_agreement / n_keyword_hits
...
def load_transactions(path: str) -> pd.DataFrame:
    ...
    df["category"] = [classify(mcc, desc) for mcc, desc in zip(df["mcc"], df["description"])]
...
def detect_streams(df: pd.DataFrame, cutoff: pd.Timestamp | None = None) -> pd.DataFrame:
    ...
    non_sub = df["description"].str.contains("|".join(NON_SUBSCRIPTION_PHRASES))
    pool = df[(df["direction"] == "out") & ~df["type"].isin(NON_SUBSCRIPTION_TYPES) & ~non_sub]
